-- =============================================
-- TRIGGER: TR_Warehouses_Audit
-- Tabla: Warehouses
-- Acciones: INSERT, UPDATE, DELETE
-- =============================================
CREATE OR ALTER TRIGGER TR_Warehouses_Audit
ON Warehouses
...
BEGIN
    ...
    IF @Action = 'DELETE'
        SELECT @RecordId = WarehouseId FROM deleted;
    ELSE
        SELECT @RecordId = WarehouseId FROM inserted;
    ...
    INSERT INTO @excludedCols VALUES ('CreatedDate'), ('CreatedBy'), ('ModifiedDate'), ('ModifiedBy');
    ...
    BEGIN
        ...
        BEGIN
            ...
            SET @isPK     = CASE WHEN @colName = 'WarehouseId' THEN 1 ELSE 0 END;
