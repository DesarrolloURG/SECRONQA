-- =============================================
-- TRIGGER: TR_Locations_Audit
-- Tabla: Locations
-- Acciones: INSERT, UPDATE, DELETE
-- =============================================
CREATE OR ALTER TRIGGER TR_Locations_Audit
ON Locations
...
BEGIN
    ...
    IF @Action = 'DELETE'
        SELECT @RecordId = LocationId FROM deleted;
    ELSE
        SELECT @RecordId = LocationId FROM inserted;
    ...
    BEGIN
        ...
        BEGIN
            ...
            SET @isPK     = CASE WHEN @colName = 'LocationId' THEN 1 ELSE 0 END;
