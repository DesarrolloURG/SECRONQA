...
CREATE OR ALTER PROCEDURE SP_Backup_SECRONDEV
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SET @RutaBase = 'C:\Program Files\Microsoft SQL Server\MSSQL16.SQLEXPRESS\MSSQL\Backup\SECRONDEV\'

    SET @FechaHora = CONVERT(NVARCHAR, GETDATE(), 112) + '_' +
                     REPLACE(CONVERT(NVARCHAR, GETDATE(), 108), ':', '')
    SET @NombreArchivo = 'SECRONDEV_' + @FechaHora + '.bak'
    ...
    BEGIN TRY
        ...
        BACKUP DATABASE SECRONDEV
        TO DISK = @RutaCompleta
        WITH FORMAT, INIT,
             NAME = 'SECRONDEV - Backup Automático'

        SELECT @TamanioMB = ROUND(size * 8.0 / 1024, 2)
        FROM sys.master_files
        WHERE name = 'SECRONDEV' AND type = 0
