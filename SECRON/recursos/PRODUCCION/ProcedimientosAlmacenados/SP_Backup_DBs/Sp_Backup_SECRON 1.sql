...
CREATE OR ALTER PROCEDURE SP_Backup_SECRON
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SET @RutaBase = 'C:\Program Files\Microsoft SQL Server\MSSQL16.SQLEXPRESS\MSSQL\Backup\SECRON\'

    SET @FechaHora = CONVERT(NVARCHAR, GETDATE(), 112) + '_' +
                     REPLACE(CONVERT(NVARCHAR, GETDATE(), 108), ':', '')
    SET @NombreArchivo = 'SECRON_' + @FechaHora + '.bak'
    ...
    BEGIN TRY
        ...
        BACKUP DATABASE SECRON
        TO DISK = @RutaCompleta
        WITH FORMAT, INIT,
             NAME = 'SECRON - Backup Automático'

        SELECT @TamanioMB = ROUND(size * 8.0 / 1024, 2)
        FROM sys.master_files
        WHERE name = 'SECRON' AND type = 0
